...
const {requireAuthTeacher, requireAuthStudent} = require('../middleware/authMiddleware');

const router = Router();

router.get("/", courseController.course_index); // all courses
router.get("/join", requireAuthStudent, courseController.join_courses); // allow students to join courses
